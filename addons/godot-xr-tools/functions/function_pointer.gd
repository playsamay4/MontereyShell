...
func _update_pointer() -> void:
	if enabled and last_target:
		_visible_hit(last_collided_at, $RayCast.get_collision_normal())
	else:
		_visible_miss()
...
func _visible_hit(at : Vector3, normal : Vector3 = Vector3.UP) -> void:
	...
	if show_target:
	# 1. Push it 1cm away from the wall so it doesn't clip
		var offset_distance : float = 0.01 
		$Target.global_transform.origin = at + (normal * offset_distance)
		
		# 2. Point the node's local -Z axis directly at the wall face
		if abs(normal.dot(Vector3.UP)) > 0.99:
			$Target.look_at(at + normal, Vector3.FORWARD)
		else:
			$Target.look_at(at + normal, Vector3.UP)
		
		# 3. FIX THE PANCAKE: Rotate the cylinder 90 degrees on its local X-axis 
		# This forces the flat Y-axis cap to snap flush against the Z-pointed wall face.
...
func _visible_move(at : Vector3) -> void:
	# ROUTE THROUGH HIT LOGIC: This ensures the cylinder retains its flush rotation 
	# and normal offset even while the controller is actively shifting.
	if $RayCast.is_colliding():
		_visible_hit(at, $RayCast.get_collision_normal())
	else:
		# Fallback if the raycast loop is slightly ahead of data updates
		_visible_hit(at, Vector3.UP)

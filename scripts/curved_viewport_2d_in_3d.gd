...
func _update_screen_size() -> void:
	# Make sure the mesh is actually loaded first
	if $Screen and $Screen.mesh:
		# 1. Automatically measure the 3D physical width/height of your curved model
		var true_bounds = $Screen.mesh.get_aabb().size
		
		# 2. Hand those exact measurements to the laser pointer math
		if has_node("StaticBody3D"):
			$StaticBody3D.screen_size = Vector2(true_bounds.x, true_bounds.y)
